...
pub fn build_probes_router() -> Router<AppState, Body> {
    Router::new()
        .route("/", post(handle_list))
        .route("/:id", get(handle_get_one))
}

#[axum_macros::debug_handler]
async fn handle_list(State(state): State<AppState>, _claim: Claims, form: Form<ListProbesData>) -> Response {
...
#[axum_macros::debug_handler]
async fn handle_get_one(State(state): State<AppState>, claim: Claims, Path(probe_id): Path<String>) -> Response {
    match  probe_id.parse::<i32>() {
        Ok(id) => {
            let result = get_probe_usecase(&state.db_pool, GetProbeData { user_id: claim.sub,probe_id: id }).await;
            match result {
                Some(res) => {ServerResponse::ok(res).into_response()},
                None => {StatusCode::NOT_FOUND.into_response()}
            }
