...
//#[derive(Params)]
//pub struct NewProbesParams {
//    pub id: i32
//}

#[component]
pub fn CreateProbePage() -> impl IntoView {
   // let params = use_params::<NewProbesParams>();

    let (name, set_name) = create_signal("probe name".to_string());
    let (probe_type, set_probe_type) = create_signal(ContainerType::Other);
    let (min, set_min) = create_signal("0".to_string());
    let (max, set_max) = create_signal("100".to_string());
    let (loading, set_loading) = create_signal( false);
    //  let (valid_form, set_valid_form) = create_signal(false);
    ...
    let creation_handler = move |ev: SubmitEvent| {
       ...
       // create_probe_action.dispatch(  (name.get(), content_type.get(), volume.get().parse::<i64>().unwrap()));
    };
            ...
            <div class="card-container">
                <div class="card">
                    ...
                    <form on:submit=creation_handler >
                        ...
                        <LabeledInput name="min" input_value=min input_return=set_min kind="number" />
                        <LabeledInput name="max" input_value=min input_return=set_max kind="number" />
                        <LabeledSelect name="type" input_value=probe_type input_return=set_probe_type options={types_options}>
                        </LabeledSelect>
                        <Show
